...
pub async fn collect(mut body: Vec<Value>, log_entry: &LogEntry) -> Vec<Value> {
    ...
    if body.len() > client.elastic_push_size as usize
        || local_time
            .signed_duration_since(datetime_with_timezone)
            .num_seconds()
            > client.elastic_push_seconds as i64
    {
...
async fn elastic_connect() -> Result<Elasticsearch, Box<dyn std::error::Error>> {
    ...
    match &cli.command {
        Send(name) => {
            ...
            let mut transport = TransportBuilder::new(conn_pool).disable_proxy();
            if name.elastic_user.is_some() {
                transport = transport.auth(Credentials::Basic(
                    name.elastic_user.clone().unwrap(),
                    name.elastic_password.clone().unwrap(),
                ));
            }
...
async fn send(body: Vec<serde_json::Value>) -> Result<Response, Box<dyn std::error::Error>> {
    ...
    let response = client
        .bulk(BulkParts::Index(
            options.elastic_index_name.unwrap().as_str(),
        ))
        .body(request_body)
        .send()
        .await?;
